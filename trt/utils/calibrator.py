...
class EntropyCalibrator(trt.IInt8EntropyCalibrator2):
    def __init__(self, data_dir, cache_file):
        ...
        self.configWidth = 512
        self.configHeight = 512
    ...
    def trt_img(self, image):
        resizeFac_h = image.shape[0]/self.configHeight
        resizeFac_w = image.shape[1]/self.configWidth
        sized = cv2.resize( image, (self.configWidth, self.configHeight))
        sized = self.shuffle_and_normalize(sized)
    ...
    def get_batch(self, names):
        ...
        for i in range(self.batch_size):
            ...
            image = cv2.imread(self.calib_imgs[self.counter + i])
            img_np = self.trt_img(image)
            img_np = img_np.ravel()
            img_np = np.ascontiguousarray(img_np)
